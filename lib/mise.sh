...
# later call costs one `mise where` per tool. A tool that is missing is
# installed the way mise_ensure_global installs one: `mise install` when the
# global config already requests it, so a pinned version survives, and
# `mise use -g` only when the config does not mention it, which writes the
# request that `teeup update`'s `mise upgrade` then keeps current. A bare
# `use -g` for both cases would drop a version the user pinned by hand the
# first time the binary went missing. The wrapper is teeup's own "install on
# first call", so it honours DRY_RUN the way `teeup lazy-run` does: the
# install is previewed and nothing is downloaded. An already-installed tool
# runs normally under DRY_RUN, because there is no mutation left to withhold. A runtime is a tool the command needs at
