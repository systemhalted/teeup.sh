...
# Ask AeroSpace whether it will accept <candidate> before anything installs it.
# teeup's own aerospace.toml is expected to pass, so this is a guard against
# shipping a config AeroSpace cannot load -- a mistake that otherwise leaves
# AeroSpace running on nothing with no message. AeroSpace can check a
