...
| The wizard order in `bootstrap` (package manager, name, email, work email, theme, daily confirm, then the Emacs flavor only after a yes, an empty answer meaning `starter`) and `WIZARD_INPUT` in `tests/bootstrap.sh` | 3a Task 2 | Task 1 (the wizard test's input) |
