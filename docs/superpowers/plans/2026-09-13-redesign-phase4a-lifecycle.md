...
16. **`remove` runs the capability's `remove` script in addition to the metadata uninstall, not instead of it.** The two `remove` scripts on `main` (`macos-defaults`, `keyboard`) and plan 3a's (`emacs`) all undo machine state and none of them touches a package, so a script that also had to uninstall its own cask would repeat what metadata already says. The script runs first, while the tool is still installed.
