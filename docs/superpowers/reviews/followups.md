...
- `teeup config edit` hints (#48 review): no test covers an edited or removed key, or pins that a dry run or rolled-back edit prints no hint; `_config_file_get` re-sources the answers file once per key; `test_update_does_not_configure_a_lazy_capability` passes before the fix too (a guard, not a regression test).
