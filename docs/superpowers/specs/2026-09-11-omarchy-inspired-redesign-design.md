...
**Amendment 2026-09-26 -- one AI command, one lazy capability.** `ai-claude`, `ai-codex`, `ai-gemini`, `ai-copilot` and `ai-opencode` each provide one command and write one mise wrapper through a shared helper. Their `ai` parent remains an explicit compatibility bundle: `teeup install ai` requests all five, while calling `claude` installs and configures only `ai-claude`. A lazy install skips every already-done requirement in its `requires=` chain but still installs a missing requirement and always repairs the requested target. Before a wrapper downloads anything it prints `Installing <tool> through mise (first run, can take a minute)...` to stderr; an interrupted request remains retryable and the next call uses the global mise request without replacing a pinned version. Lazy capability work and wrapper downloads append once to `$TEEUP_STATE_DIR/logs/lazy.log`; nested `run_logged` calls must not duplicate lines in that file or in `bootstrap.log`. A one-time migration maps an installed legacy `ai` aggregate onto the five leaf markers only when their command paths still exist, refreshes the runtime shims, and leaves foreign command files untouched. `DRY_RUN=true` previews the state and shim changes without creating a log, marker, wrapper or mise request.
...
- **Answers** (`~/.config/teeup/answers`) are the profile: identity, package manager, theme, daily set, opt-outs. Sourceable `KEY="value"` so bash 3.2 reads it with no parser. `teeup config get|set|edit` manages it. Precedence: capability defaults, then `answers`, then the machine file (machine wins because it encodes hard constraints such as the package manager or `TEEUP_SKIP`). *Amended 2026-09-18 (personal overlay; this section and section 8 first placed the machine file at the committed `machines/<hostname>.conf` alone): the machine file is now looked up in the user's own `$TEEUP_CONFIG_DIR/machines/<hostname>.conf` first and the checkout's `machines/<hostname>.conf` second (`machine_file()` in `lib/answers.sh`, and the matching lookup in `capabilities/zsh/default/env`, written in its own idiom since a shell startup path has no access to that library) -- first match wins outright, never merged, because a half-applied machine file is worse than one file that is clearly in charge, and a shadowed checkout file is announced rather than silently ignored. This is the same overlay `theme_dir` (the Themes bullet below) already gives themes, and it completes a principle already true of `answers`, the `local` files (`~/.config/zsh/local.zsh`, `~/.config/git/local`, `~/.config/wezterm/local.lua`) and themes: a user's own state lives under `~/.config/teeup` or `~/.config/<tool>`, never the checkout, so the checkout can be pulled and never forked. `machines/<hostname>.conf` in the repo still works, checked second, for anyone who keeps a fork instead.*
...
No named profiles. "Sensible defaults + my profile + machine overrides" maps to capability defaults + `answers` + the machine file. *Amended 2026-09-18 (personal overlay; see section 7): the machine file is the user's own `$TEEUP_CONFIG_DIR/machines/<hostname>.conf` when it exists, else the checkout's `machines/<hostname>.conf` -- so recording a machine setting, work identity included, never requires forking teeup.* *Amended 2026-09-17 (user decision; this section first made identity a directory rule -- see the Git, SSH/signing, Dev dirs and Work vs personal rows of the interview table): git carries one identity, full stop, so there is no `includeIf` and no per-root switching left to make a profile out of. The only thing "work" still means is a second SSH key and a second GitHub upload, and that is per-machine, not a question: `ssh` generates the work key (`id_ed25519_work`) and writes `Host github.com-work` style aliases only when `machines/<hostname>.conf` sets `TEEUP_WORK_EMAIL`; without it there is one key, one identity, no work anything. The work key is uploaded to the work identity's own GitHub host (`TEEUP_WORK_GH_HOST`, default `github.com`) and, when that is the same host as personal, to the account `TEEUP_WORK_GH_ACCOUNT` names: `gh` targets a host, and each host has one active account, so a second account on `github.com` is reached with `gh auth switch --hostname <host> --user <account>` and nothing else. Without an account named, teeup refuses the work upload rather than putting the work key on the personal account.* Machines that must skip a capability (no AeroSpace on a locked-down Mac) set `TEEUP_SKIP="aerospace"` in the machine file.
