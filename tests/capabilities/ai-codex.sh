...
test_install_is_a_dry_run_noop() {
  ...
  DRY_RUN=true cap_run ai-codex install >/dev/null 2>&1
...
test_configure_writes_nothing_in_a_dry_run() {
  ...
  local before after
  before="$(find "$TEST_HOME" -type f | sort)"
  DRY_RUN=true cap_run ai-codex configure >/dev/null 2>&1
  after="$(find "$TEST_HOME" -type f | sort)"
  assert_equals "$before" "$after" "a dry run must write nothing" || return 1
  cleanup_test_env
}

test_configure_writes_the_codex_wrapper() {
  setup
  DRY_RUN=false cap_run ai-codex configure >/dev/null
  assert_file_exists "$TEST_HOME/.local/bin/codex" || return 1
  cleanup_test_env
}

echo "capabilities/ai-codex"
...
run_test "configure writes the codex wrapper" test_configure_writes_the_codex_wrapper
print_summary
