...
teeup.sh is released under the [MIT License](LICENSE).

Dotfiles generated by `--init-dotfiles` are yours: the starter files teeup writes
into your dotfiles repo carry no teeup copyright and no attribution requirement.
License them however you like, or not at all.

teeup installs third-party tools (Homebrew, Oh My Zsh, Powerlevel10k, Starship,
SDKMAN!, rustup, and others) by fetching them at run time on your machine. None
of that code is bundled here; each remains under its own license.
