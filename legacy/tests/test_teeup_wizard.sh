...
test_dotfiles_step() {
  ...
  assert_contains "$content" "WIZARD_DOTFILES_MANAGER" "Should track the detected dotfiles manager" || return 1
  assert_contains "$content" "(will be installed)" "Should warn when the manager binary is missing" || return 1
