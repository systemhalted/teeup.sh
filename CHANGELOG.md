...
### Added
- **MIT license.** The project now ships a `LICENSE` file (MIT, © 2026 Palak
  Mathur) and a license section in the README. Dotfiles generated by
  `--init-dotfiles` are explicitly exempt: the starter files teeup writes into
  your dotfiles repo carry no teeup copyright and no attribution requirement, so
  a permissive project license never encumbers the output. Third-party tools
  teeup installs are fetched at run time and stay under their own licenses;
